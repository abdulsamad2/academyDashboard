...
export default function OnboardingForm() {
  ...
  if (session?.onboarding !== true && session?.role === 'parent')
    router.push('/parent-dashboard');
  if (session?.onboarding !== true && session?.role === 'tutor')
    router.push('/tutor-dashboard');
  if (session?.onboarding !== true && session?.role === 'admin')
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-100 p-4">
      <Card className="w-full max-w-screen-lg ">
        <CardHeader>
          ...
          <CardDescription>
            Please provide the following information to get started with our
            tuition academy.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <RadioGroup
              defaultValue="parent"
              onValueChange={(value) =>
                setUserType(value as 'parent' | 'tutor')
              }
          ...
          {userType === 'parent' && <ParentOnBoarding initialData={null} />}

          {userType === 'tutor' && (
            <TutorOnboarding
              initialData={null} //@ts-ignore
