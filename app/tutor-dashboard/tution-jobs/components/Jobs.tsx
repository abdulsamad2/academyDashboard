...
export default function Jobs({ tutorRequests, currentTutorId }: JobsProps) {
  ...
  const onSubmit = async (data: ApplicationFormData) => {
    try {
      ...
      reset();
      router.push('/tutor-dashboard/tution-jobs');
    } catch (error) {
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <DetailRow
              icon={BadgeHelp}
              label="Student Level"
              value={request.studentLevel}
            />
            <DetailRow
              icon={Calendar}
              label="Start Date"
              value={new Date(request.start).toLocaleDateString()}
            />
            <div className="col-span-full">
              <DetailRow
                icon={MapPin}
                label="Location"
                value={request.location || 'online'}
              />
            </div>
            <DetailRow
              icon={Banknote}
              label="Hourly Rate"
              value={`${request.hourly}/hr`}
            />
            <DetailRow
              icon={User}
              label="Student Age"
              value={request.studentAge}
            />
            <DetailRow
              icon={Calendar}
              label="Days Available"
              value={request.dayAvailable}
            />
            <DetailRow
              icon={Clock}
              label="Time Range"
            ...
            <DetailRow
              ...
              value={request.sessionsPerMonth}
            />
          </div>

          <div>
            <h4 className="mb-2 font-semibold">Requirements & Details</h4>
    ...
    <div className="container mx-auto max-w-7xl px-4 py-8">
      ...
      <div className="grid gap-4 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {filteredRequests.map((request) => {
          const hasApplied = request.application.some(
            (app: any) => app.tutorId === currentTutorId
          );
          return (
            <Card
              key={request.id}
                    ...
                    <DialogContent className="sm:max-w-[525px]">
                      <DialogHeader>
                        <DialogTitle className="flex items-center space-x-2">
                          ...
                          <span>
                            Apply for {request.subject} Tutor Position
                          </span>
                        </DialogTitle>
                      </DialogHeader>
