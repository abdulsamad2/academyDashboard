...
/**
 * Verify (tutorId, studentId) is a valid assignment.
 * Admins bypass — they can log lessons on behalf of any tutor.
 */
async function assertAssigned(
  ...
  if (!studentId || !tutorId) return 'Student and tutor are required';
  const assignment = await db.studentTutor.findFirst({
    where: { studentId, tutorId }
  });
  if (!assignment) {
    return 'You can only log lessons for students assigned to you';
  }
  return null;
}
    ...
    const res = await db.lesson.update({
      ...
      data: {
        ...
        date: lessonData.date
          ? new Date(lessonData.date).toISOString()
          : undefined,
...
export const getLessonForThisTutorAndStudent = async (
  tutorId: string,
  studentId: string
) => {
  try {
...
export const getTotalDurationForStudentandTutorThisMonth = async (
  studentId: string,
  tutorId: string
) => {
  try {
