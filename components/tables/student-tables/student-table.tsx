...
export function StudentTable<TData, TValue>({
  ...
  useEffect(() => {
    ...
    // This is a no-op but keeps React happy about the dependency array
    if (columns) {
      // Do nothing, just dependency tracking
    }
  }, [columns]);
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Manage and monitor your student profiles
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row items-center gap-4 justify-between bg-muted/5 p-4 rounded-xl">
        <div className="flex items-center gap-2 text-sm text-muted-foreground order-2 md:order-1">
          <span className="font-medium text-foreground">{table.getFilteredRowModel().rows.length}</span> of <span className="font-medium text-foreground">{data.length}</span> students
        </div>
